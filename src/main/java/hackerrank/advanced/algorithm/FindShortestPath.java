...
public class FindShortestPath {
    ...
    public static void getShortestPath(int[][] graph, int sourceVertex) {
	...
	for (int i = 0; i < vertexCount; i++) {
	    // find the neighbouring unvisited vertex having minimum distance from source
	    // vertex
	...
	for (int i = 0; i < distance.length; i++) {
	    System.out.println(
		    String.format("Distance from source vertex %s to vertex %s is %s", sourceVertex, i, distance[i]));
	}
